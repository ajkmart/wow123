...
if [ -z "$DB_URL" ]; then
  ...
  # ── Track 1: Custom SQL migrations (lib/db/migrations/) ──────────────────
  # Uses _schema_migrations table (same as sqlMigrationRunner.ts at runtime)
  psql "$DB_URL" -c "
  ...
  # ── Track 2: Drizzle-generated migrations (lib/db/drizzle/) ──────────────
  # Uses _drizzle_migrations table (same as sqlMigrationRunner.ts at runtime)
  psql "$DB_URL" -c "
