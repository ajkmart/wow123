...
router.get("/transactions", async (req, res) => {
  ...
  const rows = await db
    ...
    .where(cursor ? sql`${walletTransactionsTable.createdAt} < ${cursor}::timestamptz` : undefined)
  ...
  const totalCredit = page.data.filter(t => t.type === "credit").reduce((s, t) => s + parseFloat(t.amount), 0);
  const totalDebit  = page.data.filter(t => t.type === "debit").reduce((s, t)  => s + parseFloat(t.amount), 0);
